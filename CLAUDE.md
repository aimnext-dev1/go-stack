...
`docker compose` 스택을 배포/관리/백업/복원하는 단일 Go 바이너리 CLI `dtx`.
...
- `init.go` — `dtx init` (stack.env 템플릿 생성 + .gitignore 등록)
...
1. **버그 3개 수정**
   ...
   - `checkStackExists()`: `ps -a`(헤더 때문에 항상 비어있지 않음) → `ps -aq`로 수정. `dtx up`이 영구 차단되던 버그
   - `dtx connect`: shell 문자열 통짜를 `run()`에 단일 인자로 넘기던 것 → `"sh","-c",script` 로 argv 분리
2. **`dtx init` 명령어 추가**: 현재 폴더에 `stack.env` 템플릿 생성(항목별 설명 주석 포함) + `.gitignore`에 자동 등록. `_project/` 폴더는 만들지 않음. 이미 `stack.env` 있으면 에러 후 아무것도 안 바꿈.
3. **인자 없이 `dtx` 실행 시 help 출력**: `loadConfig()`(stack.env 필수) 호출 전에 처리하도록 순서 변경 — stack.env 없는 곳에서도 `dtx`/`dtx help`/`dtx init` 동작
...
- 코드 내 사용자 대상 문자열(에러/로그): 전부 한글. 기술 식별자(`STACK_NAME`, `stack.env`, `dtx` 등)는 원문 유지
