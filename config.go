...
func loadConfig() {
    ...
    if cfg.stackName == "" { fatal("STACK_NAME is not set in stack.env") }
...
func findRoot() string {
    ...
    for {
        ...
        if parent == dir { fatal("stack.env not found (searched from current folder upward)") }
        dir = parent
    }
}
...
func parseEnvFile(path string) {
    ...
    if err != nil { fmt.Fprintf(os.Stderr,"failed to read stack.env: %v\n",err); os.Exit(1) }
